            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Real-time Mothly & Yearly Updates</h1>
                        <a href="#" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"> <i class="fas fa-clipboard-list fa-2x text-gray-300"></i> General Report</a>
                    </div>
                                    ...
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-primary text-uppercase mb-1">
                                                Sales</div>
                                           <div class="h5 mb-0 font-weight-bold text-gray-800">Ksh {{ number_format($sales, 2) }}</div>
                                        </div>
                                        <div class="col-auto">
                        ...
                        <div class="col-xl-3 col-md-6 mb-4">
                            <div class="card border-left-primary shadow h-100 py-2">
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-primary text-uppercase mb-1">Repairs
                                            </div>
                                            <div class="row no-gutters align-items-center">
                                                <div class="col-auto">
                                                    <div class="h5 mb-0 font-weight-bold text-gray-800">{{ number_format($repairs, 2) }}</div>
                                                </div>
                                    ...
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-warning text-uppercase mb-1">
                                                 Photocopies</div>
                                            <div class="h5 mb-0 font-weight-bold text-gray-800">Ksh {{ number_format($photocopies, 2) }} </div>
                                        </div>
                                        <div class="col-auto">
                                ...
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            ...
                                            <div class="row no-gutters align-items-center">
                                                <div class="col-auto">
                                                    <div class="h5 mb-0 font-weight-bold text-gray-800">Ksh{{ number_format($printing, 2) }}</div>
                                                </div>
                          ...
                          <div class="col-xl-3 col-md-6 mb-4">
                            <div class="card border-left-info shadow h-100 py-2">
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-info text-uppercase mb-1">OTHER SERVICES

                                            </div>
                                            <div class="row no-gutters align-items-center">
                                                <div class="col-auto">
                                                   <div class="h5 mb-0 font-weight-bold text-gray-800">Ksh{{ number_format($other_services, 2) }}</div>
                                                </div>
                                ...
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            ...
                                            <div class="row no-gutters align-items-center">
                                                <div class="col-auto">
                                                    <div class="h5 mb-0 font-weight-bold text-gray-800">Ksh{{ number_format($profit, 2) }}</div>
                                                </div>
                                ...
                                <div class="card-body">
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            <div class="text-xs font-weight-bold text-warning text-uppercase mb-1">
                                                 EXPENSES</div>
                                            <div class="h5 mb-0 font-weight-bold text-gray-800">Ksh	{{ number_format($expenses, 2) }}</div>
                                        </div>
                                        <div class="col-auto">
                                            <i class="fas fa-dollar-sign fa-2x text-gray-300"></i>
                                        </div>
...
<!-- Leave your <head> and <body> untouched -->

<!-- Chart.js -->
<!-- Radio toggle -->
<!-- resources/views/admin/admin_graph.blade.php -->
 ...
 <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
  ...
  <div class="toggle-container">
    ...
    <input type="radio" name="viewToggle" value="monthly" checked onchange="renderAllCharts('monthly')"> Monthly
    <input type="radio" name="viewToggle" value="yearly" onchange="renderAllCharts('yearly')"> Yearly
  </div>
...
<div class="container">
    ...
    <div class="graph-card">
        <h2>Other Services Graph</h2>
        <div class="chart-container">
            <canvas id="profitChart"></canvas>
        </div>
    </div>
    <div class="graph-card">
        <h2>Repairs Revenue</h2>
        <div class="chart-container">
            <canvas id="repairsChart"></canvas>
        </div>
    </div>
    <div class="graph-card">
        <h2>Photocopies Revenue</h2>
        <div class="chart-container">
            <canvas id="photocopiesChart"></canvas>
        </div>
    </div>
    <div class="graph-card">
        <h2>Printing Revenue</h2>
        <div class="chart-container">
...
<script>
    ...
    function createOrUpdateChart(id, label, data, bgColor, labels) {
        const ctx = document.getElementById(id).getContext('2d');

        if (chartRefs[id]) {
            chartRefs[id].data.labels = labels;
            chartRefs[id].data.datasets[0].data = data;
            chartRefs[id].update();
        } else {
    ...
    async function fetchChartData(view = 'monthly') {
        try {
            const res = await fetch(`/admin/graphs/${view === 'monthly' ? 'getMonthlyData' : 'getYearlyData'}`);
    ...
    async function renderAllCharts(view = 'monthly') {
        const data = await fetchChartData(view);
        if (!data) return;

        const labels = view === 'monthly' ? months : data.labels;

        createOrUpdateChart('salesChart', 'Sales', data.sales, '#4e73df', labels);
        createOrUpdateChart('stockChart', 'Profit', data.profit, '#1cc88a', labels);
        createOrUpdateChart('profitChart', 'Other Services', data.other_services, '#36b9cc', labels);
        createOrUpdateChart('repairsChart', 'Repairs', data.repairs, '#f6c23e', labels);
        createOrUpdateChart('photocopiesChart', 'Photocopies', data.photocopies, '#e74a3b', labels);
        createOrUpdateChart('printingChart', 'Printing', data.printing, '#858796', labels);
        createOrUpdateChart('expensesChart', 'Expenses', data.expenses, '#20c997', labels);
    }

    // Initial chart load
    renderAllCharts();

    // Real-time auto-refresh every 30 seconds (adjust as needed)
    setInterval(() => renderAllCharts(), 30000);

    // Toggle handler for switching between monthly and yearly
    document.querySelectorAll('input[name="toggle"]').forEach(radio => {
        radio.addEventListener('change', () => {
            renderAllCharts(radio.value);
        });
    });
</script>
