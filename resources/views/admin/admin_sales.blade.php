...
<div class="card shadow mb-4">
    <div class="card-header py-3 d-flex justify-content-between align-items-center">
        <h6 class="m-0 font-weight-bold text-primary">SalesTable</h6>
        ...
        <tbody>
            ...
            @forelse($sales as $sale)
                @php
                    ...
                    $buyingPrice = $sale->product->buying_price ?? 0;
                    $profit = ($sale->price_per_unit - $buyingPrice) * $sale->quantity;
                    $grandProfit += $profit;
                @endphp
                <tr>
                    ...
                    <td>{{ $sale->quantity }}</td>
                    <td>Ksh {{ number_format($sale->price_per_unit, 2) }}</td>
                    <td><strong>Ksh {{ number_format($sale->total_price, 2) }}</strong></td>
                    <td><span class="text-success">Ksh {{ number_format($profit, 2) }}</span></td>
                    <td>{{ ucfirst($sale->payment_method ?? '-') }}</td>
                    <td>
                        @if($sale->status === 'paid')
                            <span class="badge bg-success">Paid</span>
                        @else
                            <form method="POST" action="{{ route('admin.sales.markPaid', $sale->id) }}">
                                @csrf
                                @method('PATCH')
                                <div class="input-group">
                                    <select name="payment_method" class="form-select form-select-sm bg-amber-500" required>
        ...
        @if(count($sales) > 0)
            <tfoot>
                <tr>
                    ...
                    <td><strong class="text-success">Ksh {{ number_format($grandProfit, 2) }}</strong></td>
                    <td colspan="4"></td>
                </tr>
            </tfoot>
        @endif
